...
[Single-file purge](/cache/how-to/purge-cache/purge-by-single-file/) may not work as expected in the following situations:

- If your Cache Rule sets a [custom cache key](/cache/how-to/cache-keys/), because the dashboard cannot send the headers or cookies that are part of the key.
- If your rule expression matches only `GET` requests (for example, `http.request.method eq "GET"`), because purge requests use a different HTTP method internally.
